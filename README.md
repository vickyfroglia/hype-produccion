...
- **Permisos por campo en Producción.** Aunque todos los roles ven la tabla completa, cada uno solo puede editar los campos de su parte del circuito (el resto queda deshabilitado, en gris):
  ...
  - **Pedido terminado (Fecha fin cargada):** la fila se "congela" — nadie salvo `admin` puede editar ningún campo de esa fila, aunque normalmente le correspondiera a su rol. **Excepción:** el propio botón de Fecha fin queda accesible para quien tiene permiso sobre ese campo (encargado, logística, admin), para poder revertir "terminado" y volver a abrir el pedido.
  - **"Terminado" ahora es una acción explícita**, no automática. Antes, cargar Op Fij ponía sola la Fecha fin y congelaba todo de golpe (sin forma de revertir Op Imp/Op Fij). Ahora Op Imp y Op Fij tienen su opción vacía y se pueden revertir libremente mientras el pedido no esté marcado como terminado. Recién cuando se aprieta "✓ Marcar" en la columna Fecha fin (requiere tener Op Fij cargado) se pone la fecha de hoy y se congela la fila. Con "revertir" se vuelve a abrir.
...
- **Administración: solo pendientes de anticipo + reporte de OTs terminadas.** La pantalla ya no muestra "todos los pedidos", solo los que tienen anticipo PENDIENTE (arranca así por default al cargar el pedido). Abajo hay un segundo reporte, "Trabajos terminados, listos para avisar al cliente": agrupa por Nro OT y solo aparece cuando **todos** los diseños de esa OT tienen Fecha fin cargada (no alcanza con uno solo). Cada tarjeta tiene botones "Copiar" (arma un texto con diseño/mts impresos/tela, listo para pegar en WhatsApp), "Imprimir" (abre una hoja simple para imprimir) y "Cliente avisado" (tilda la OT completa y desaparece de la lista — columna nueva `cliente_avisado`).
...
- **Reordenar prioridad (columna N) en Producción.** Solo `admin` ve las flechas ▲▼ junto al número N. Mueven la OT completa (todos sus diseños juntos) un lugar arriba o abajo — el resto de las OT se reacomoda solo. Se guarda en una columna nueva, `orden_manual`, que define el orden real (antes se calculaba solo por fecha/id, sin forma de forzarlo a mano). Los pedidos nuevos entran siempre al final de la lista.
...
- Cuando Impresión carga los **mts impresos** de una tela de **cliente**, se genera automáticamente un **egreso real en Stock** (tabla `egresos`, estado "A producción") por esa cantidad — el stock se descuenta solo, no hay que ir a cargarlo de nuevo en la app de Stock. Ese egreso queda vinculado a la OT (`orden_id`).
- Para telas **HYPE (Stock TH, código que arranca con "TH")** el egreso se genera antes: apenas se guarda el pedido en Nuevo Pedido, por los **mts pedidos** (no hay que esperar a que se imprima). Esto es porque el stock TH es compartido entre pedidos, y así se ve el stock comprometido y lo que falta conseguir desde el momento en que entra el pedido. Al cargar después los mts impresos de esa misma OT, no se vuelve a descontar (ya quedó reservado).
