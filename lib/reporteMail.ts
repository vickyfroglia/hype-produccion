...
// ---------------------------------------------------------------------------
// Lógica compartida por los 3 reportes de productividad por mail (diario,
// semanal, mensual — ver app/api/reporte-diario, reporte-semanal y
// reporte-mensual). Cada uno solo calcula su propio rango de fechas y su
// asunto/título, y le pasa todo a `generarYEnviarReporte`.
//
// Usa el Service Role de Supabase (no el anon key) porque estos endpoints
// corren sin un usuario logueado, y las políticas RLS de reporte_rollos
// exigen auth.role() = 'authenticated'. El Service Role se salta RLS — por
// eso NUNCA debe tener el prefijo NEXT_PUBLIC_ (quedaría expuesto al
// navegador si lo tuviera).
// ---------------------------------------------------------------------------

export const OFFSET_ART_MS = 3 * 60 * 60 * 1000; // Argentina = UTC-3 todo el año (sin horario de verano)

const GRUPO_FIJADO = ['FIJADO', 'FIJ Y POSTRATADO'];
const GRUPO_PREP_PLANCHADO = ['PREP Y PRETRATADO', 'PLANCHADO', 'PREP Y REENCANUTADO'];

// Fecha (YYYY-MM-DD) de "hoy" en hora Argentina, más un corrimiento en días
// (negativo = para atrás). Así el cron corre en UTC (Vercel) pero las
// fechas del reporte siempre se calculan como se ven desde acá.
export function fechaArgentina(offsetDias = 0): string {
  const ahoraArt = new Date(Date.now() - OFFSET_ART_MS);
  ahoraArt.setUTCDate(ahoraArt.getUTCDate() + offsetDias);
...
function armarHtml(tituloPrincipal: string, subtitulo: string, mona32: { operario: string; mts: number }[], mona8: { operario: string; mts: number }[], term: { operario: string; fijado: number; prep: number }[]): string {
  ...
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;">
    <div style="background:#1a1a2e;color:#ffffff;padding:20px 24px;border-radius:8px 8px 0 0;">
      ...
      <div style="font-size:20px;font-weight:700;margin-top:4px;">${tituloPrincipal}</div>
      <div style="font-size:13px;color:#cfcfe0;margin-top:2px;">${subtitulo}</div>
    </div>
    <div style="padding:22px 20px;background:#fdfbf5;border:1px solid #eee;border-top:none;">
      ...
      <div style="font-size:11px;color:#999;margin-top:6px;">
        Reporte generado automáticamente a partir de Reporte diario (mts entre ${subtitulo}).
      </div>
    </div>
  </div>`;
}
...
export interface OpcionesReporte {
  ...
  fechaDesde: string; // YYYY-MM-DD, inclusive
  fechaHasta: string; // YYYY-MM-DD, inclusive
  tituloPrincipal: string; // ej: "Reporte semanal de producción"
  subtitulo: string; // ej: "Semana del 20/07 al 26/07"
  asunto: string; // asunto completo del mail, ej: "REPORTE SEMANAL — 20/07 al 26/07"
}

// Núcleo compartido: valida el secret, consulta reporte_rollos en el rango
// de fechas dado, arma el HTML y manda el mail. Devuelve directo un
// NextResponse listo para retornar desde el route handler.
export async function generarYEnviarReporte(opts: OpcionesReporte): Promise<NextResponse> {
  const secret = opts.req.nextUrl.searchParams.get('secret');
  ...
  const { data, error } = await supabaseAdmin
    .from('reporte_rollos')
    .select('equipo, op_imp, mts_imp_rollo, op_fij, mts_fij, tipo_proceso, fecha')
    .gte('fecha', opts.fechaDesde)
  ...
  const html = armarHtml(opts.tituloPrincipal, opts.subtitulo, mona32, mona8, term);
  ...
  try {
    await transporter.sendMail({
      from: `"HYPE Printlab — Reporte" <${process.env.GMAIL_USER}>`,
      to: destinatarios,
      subject: opts.asunto,
      html,
    });
  ...
  return NextResponse.json({ ok: true, fechaDesde: opts.fechaDesde, fechaHasta: opts.fechaHasta, destinatarios });
}
